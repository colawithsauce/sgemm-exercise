...
__global__ void gemm_kernel3(float *d_A, float *d_B, float *d_C, int M, int N, int K, int m, int n, int k)
{
    ...
    // 注意这里全是 x，因为下面的测试代码里，只设置了 blockDim.x
        ...
        for (int k_reg_index = 0; k_reg_index < k; k_reg_index += reg_size)
        {
            for (int i = 0; i < reg_size; i++)
            {
                for (int j = 0; j < reg_size; j++)
                {
                    ...
                    {
                        reg_C[i][j] += A_sh[m_index * reg_size * k + k_reg_index + i * k + k_index] *
                                       B_sh[k_reg_index * n + n_index * reg_size + k_index * n + j];
                    }
                }
            }
        }
    ...
    for (int i = 0; i < reg_size; i++)
    {
        ...
        {
            int C_index =
                (M_tile_index * m + m_index * reg_size) * N + N_tile_index * n + n_index * reg_size + i * N + j;
            if (C_index < M * N)
            {
                // printf("C_index: %d \n", C_index);
...
__global__ void gemm_kernel3_5(float *d_A, float *d_B, float *d_C, int M, int N, int K, int m, int n, int k)
{
    ...
    //     // printf("readA1: %lf %lf %lf %lf , readA2: %lf %lf %lf %lf ,,, %lf\n", read1.x, read1.y, read1.z, read1.w,
    //     read2.x, read2.y, read2.z, read2.w, d_A[d_A_index + K_tile_index * k]); global_load<0>(&d_A[d_A_index +
    //     (K_tile_index + 1) * k + 4], read[1]); vmcnt<0>();
    //     // printf("readA1: %lf %lf %lf %lf , readA2: %lf %lf %lf %lf ,,, %lf\n", read1.x, read1.y, read1.z, read1.w,
    //     read2.x, read2.y, read2.z, read2.w, d_A[d_A_index + K_tile_index * k]); A_sh[A_sh_offset + (A_n_index + 4) *
    //     (m + padding) + A_m_index] = read[1].x; A_sh[A_sh_offset + (A_n_index + 5) * (m + padding) + A_m_index] =
    //     read[1].y; A_sh[A_sh_offset + (A_n_index + 6) * (m + padding) + A_m_index] = read[1].z; A_sh[A_sh_offset +
    //     (A_n_index + 7) * (m + padding) + A_m_index] = read[1].w;
    // } else {
    ...
    // read2.x, read2.y, read2.z, read2.w, d_B[d_B_index + K_tile_index * k * N]);
    // }
    ...
    //             printf("readA1: %lf %lf %lf %lf , readA2: %lf %lf %lf %lf ,,, %lf\n", read[0].x, read[0].y,
    //             read[0].z, read[0].w, read[1].x, read[1].y, read[1].z, read[1].w, d_A[d_A_index + K_tile_index * k]);
    //         } else {
    //             vmcnt<0>();
    //             FLOAT4(B_sh[B_sh_offset + B_m_index * (n + padding) + B_n_index]) = read[0];
    //             vmcnt<0>();
    //             FLOAT4(B_sh[B_sh_offset + B_m_index * (n + padding) + B_n_index + 4]) = read[1];
    //             printf("readB1: %lf %lf %lf %lf , readB2: %lf %lf %lf %lf ,,, %lf\n", read[0].x, read[0].y,
    //             read[0].z, read[0].w, read[1].x, read[1].y, read[1].z, read[1].w, d_B[d_B_index + (K_tile_index + 1)
    //             * k * N]);
    //         }
    // }
    // } while (K_tile_index < int((K + k - 1) / k));
